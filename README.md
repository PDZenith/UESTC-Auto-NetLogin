...
本项目的核心目标是解决 **假期/长期无人值守** 场景下，实验室电脑因断电、死机重启后，无法自动通过校园网网页认证（Captive Portal），导致远程控制软件（RayLink/ToDesk/Sunshine）彻底失联的痛点。
...
软件装好了只是第一步，要实现“拔掉电源也能自动上线”，必须进行以下 BIOS 和 Windows 设置。

### 1. BIOS 设置 (物理防断电)
*此步骤保证电脑在通电后能自动开机，无需按机箱电源键。*
...
*(其他品牌主板请寻找 `AC Recovery` 或 `AC Back` 选项)*

### 2. Windows 系统设置 (防蓝屏卡死)
